...
class Process_mass():
    ...
    def __init__(self, tab_manager=None):
        ...
        self.df = tab_manager.get_df_cas(keepcodes='',
                                         removecodes='')
    ...
    def _get_carrier_names(self):
        ...
        self.df = pd.merge(self.df,cn[['Purpose','is_carrier']],on='Purpose',
    ...
    def _mark_singular_carriers(self):
        ...
        self.df = pd.merge(self.df,gb[['UploadKey','singular_carrier']],on='UploadKey',
                           how='left',validate='m:1')
    ...
    def _get_total_event_mass(self):
        ...
        mg = pd.merge(gb1,gb2,on='UploadKey',how='left',validate='1:1')
    ...
    def run(self):
        ...
        self.tab_man.tables['allrec'].replace_df(self.df,
                                       added_fields=['bgMass','is_carrier'])
        gb = self.df.groupby('iUploadKey',as_index=False)['total_mass'].first()
        self.tab_man.tables['event'].merge_df(gb)
